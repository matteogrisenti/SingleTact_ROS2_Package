...
class SingleTactInterface:
    def __init__(self, port_driver, i2c_address=0x04):
        ...
        self.cmd_id = 0
        self.register_force = 128       # The Start Register for the force reading in the sensor's I2C map
        self.bytes_to_read = 6          # We need to read 6 bytes to get the full force reading (frame index, timestamp, digital output)
    ...
    def read_force(self):
        ...
        cmd = SerialCommand.generate_read_command(
            self.i2c_address,       # I2C address of the sensor (default: 0x04)
            self.cmd_id,            # Command ID (incremented with each command sent, modulo 256)
            self.register_force,    # Register from which to read the force value (128 is the start register for the force reading in the sensor's I2C map)
            self.bytes_to_read      # Number of bytes to read (6 bytes for the full force reading: frame index, timestamp, digital output)
        )
        ...
        if data_payload_bytes is not None and len(data_payload_bytes) >= 6:
            ...
            # 2. Extract the DIGITAL OUTPUT (Byte 4 and Byte 5 of the data payload)
            ...
            # Convert to signed 16-bit integer (handles negative values after tarering)
